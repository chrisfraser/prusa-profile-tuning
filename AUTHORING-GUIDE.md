...
- **Don't invent a numeric threshold to drive a flag.** An early L19 flag tried to detect a Buddy `S` value
  pasted into an MK3S row by testing `< 0.02`; the K and S ranges genuinely overlap (L12: MK3S PETG K 0.08,
...
- L9 `0009-import-the-starter-profiles.html`
- L10 `0010-the-temperature-tower.html`
- L11 `0011-extrusion-multiplier.html`
- L12 `0012-pressure-and-linear-advance.html`
- L13 `0013-retraction.html`
- L14 `0014-cooling.html`
- L15 `0015-max-volumetric-speed.html`
- L16 `0016-speeds-and-accelerations.html`

**Phase 3 · Ship it** — `#phase-3`
- L17 `0017-profile-structure-and-naming.html`
- L18 `0018-the-validation-set.html`
- L19 `0019-the-tuning-log.html`

Phase 3 was written **before** Phase 2, at the learner's request. Phase 2 landed 2026-08-05 and L17's
`nav.lesson-nav` prev has been repointed from `../index.html#phase-2` to `0016-*.html`. The course is now
a continuous 19-lesson chain with no gaps.

**2026-08-05, later the same day: L9 (import the starter profiles) was inserted at the head of Phase 2**,
shifting the old lessons 9–18 to 10–19 (files renamed, all cross-references updated).
`assets/widgets.js` carries a one-time localStorage migration (`migrateL9Insert`) that shifts stored
progress and checklist keys to match — don't remove it.

## Downloadable prints (`downloads/`)

Every model a lesson asks the learner to print is generated by `tools/generate-stls.py` into
`downloads/` — purpose-built, licence-free, dimensioned to match the lesson text (band heights against
the sample G-code, the 30 × 30 × 12 EM box, 40 mm spike spacing, and so on). Lessons link them with a
`div.callout.build` labelled **"⬇ Download · …"** placed next to the paragraph that specifies the print.
...
and the guide is left as originally written — see L13 §4 (flexibles retraction, where Prusa and the guide
genuinely disagree) and L15 §4 (the KB's hotend table predating the MK4S and Core One).
